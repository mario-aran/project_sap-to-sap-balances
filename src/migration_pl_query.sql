WITH
  ...
  calc_ceco AS (
    SELECT
      ...
      CASE
        WHEN NULLIF(OCR1."PrcCode", '') IS NULL THEN NULL
        WHEN ccm."MappedId" IS NOT NULL THEN NULL
        WHEN pcm."MappedId" IS NOT NULL THEN pcm."MappedId"
        ELSE 'NOT MAPPED'
      END AS "ProfitCenter",
      CASE
        WHEN NULLIF(OCR1."PrcCode", '') IS NULL THEN NULL
        WHEN pcm."MappedId" IS NOT NULL THEN NULL
        WHEN ccm."MappedId" IS NOT NULL THEN ccm."MappedId"
        ELSE 'NOT MAPPED'
      END AS "CostCenter",
      ROUND(
        re."LineAmount" * COALESCE(OCR1."PrcAmount" / OOCR."OcrTotal", 1),
        0
      ) AS "LineAmountSplit"
    FROM
      reconciled_entries re
      LEFT JOIN OOCR ON OOCR."OcrCode" = re."ProfitCode"
